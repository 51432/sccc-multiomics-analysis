...
# base dirs
resources_dir=/data/person/wup/liusy/resources
software_dir=/data/person/wup/liusy/software
genomes=/data/person/wup/liusy/reference_genomes
...
if [[ -z $1 ]]; then
    ...
    export reference_dict=${genomes}/hg38/gatk_bundle/hg38.dict
    ...
    export knownsites_snps=${genomes}/hg38/gatk_bundle/1000G_phase1.snps.high_confidence.hg38.vcf.gz  #vcf.gz
    ...
    export knownsites_indels=${genomes}/hg38/gatk_bundle/Mills_and_1000G_gold_standard.indels.hg38.vcf.gz  #vcf.gz
    ...
    export gatk_pon_location=${genomes}/${2}/gatk_bundle/PoNs            #注意
    # reference independent locations:
    # path to snpEff jar file
    export snpeff_jar=${software_dir}/snpEff/snpEff.jar
    # path to snpEff data dir
    export snpeff_datadir=${software_dir}/snpEff/data    #注意
    # path to vep data dir
    export vep_datadir=/data/person/wup/liusy/software/ensembl-vep/cache113   #注意
    export vep_species="homo_sapiens"
    # path to varscan jar file
    export varscan_jar=${software_dir}/varscan/VarScan.v2.4.6.jar
    # point to recent version of gatk
    export gatk_path=${software_dir}/gatk-4.4.0.0
    ...
    export annovar_db=/data/person/wup/liusy/software/ANNOVAR/humandb
else
    # test organism
    if [[ ${1} == "human" ]]; then
        ...
        if [[ ${2} == "hg38" ]]; then
            ...
            if [[ ${3} == "wes" ]]; then
                ...
                # path to WGS target intervals
                export intervals=${genomes}/${2}/AgilentSureSelectV5/wgs_calling_regions.hg38.interval_list
				# path to WGS tergets in bed format
                export intervals_bed=${genomes}/${2}/AgilentSureSelectV5/chr1_chr22_intervals.bed                             #wgs_calling_regions.hg38.bed
                # path to WGS intervals for running MuTect2
                export bed30intervals=${genomes}/${2}/AgilentSureSelectV5/wgs_calling_regions.hg38.30-bed-files/
            fi
            # reference independent locations:
            # path to snpEff jar file
            export snpeff_jar=${software_dir}/snpEff/snpEff.jar
            # path to snpEff data dir
            export snpeff_datadir=${software_dir}/snpEff/data  
            # path to varscan jar file
            export varscan_jar=${software_dir}/varscan/VarScan.v2.4.6.jar
            # point to recent version of gatk
            export gatk_path=${software_dir}/gatk-4.4.0.0
